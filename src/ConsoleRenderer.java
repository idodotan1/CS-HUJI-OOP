...
//	private static final int BUFFER_ROWS = board.getSize() * ROWS_PER_MARK + (board.getSize()-1) +
//	NUM_ROWS_BEFORE_BOARD;
//	private static final int BUFFER_COLS = board.getSize() * COLS_PER_MARK + (board.getSize()-1) +
//	NUM_COLS_BEFORE_BOARD;
    ...
    public ConsoleRenderer(int size) {
        ...
        for (int col = NUM_COLS_BEFORE_BOARD + COLS_PER_MARK / 2; col < bufferCols; col +=
                COLS_PER_MARK + 1)
            buffer[NUM_ROWS_BEFORE_BOARD / 2][col] = index++;
        index = FIRST_ROW; // start counting row indexes here: 00
        for (int row = NUM_ROWS_BEFORE_BOARD + ROWS_PER_MARK / 2; row < bufferRows;
             row += ROWS_PER_MARK + 1) {
